...
调用者（memspace, kexec, ...）
   │
   │ safe API: PageTable64::query/modify, PageTableMut::map/unmap/remap/protect
   │
   v
┌──────────────────────────────────────────────────┐
│  page_table                                      │
...
- **错误码**：`PtError` 枚举覆盖 6 种故障场景（`NoMemory`、`NotAligned`、`NotMapped`、`AlreadyMapped`、`MappedToHugePage`、`InvalidAddress`），`feature = "kerrno"` 时可转换为 `KError`。
