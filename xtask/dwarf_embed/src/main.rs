...
use std::{env, error::Error, fs, ops::Range, path::PathBuf};
...
fn main() {
    if let Err(err) = run() {
        eprintln!("dwarf_embed failed: {err}");
        std::process::exit(1);
    }
}
...
fn parse_args() -> Result<PathBuf, Box<dyn Error>> {
    ...
    let Some(path) = args.next() else {
        return Err(format!("Usage: {} <elf-file>", program.to_string_lossy()).into());
    };
    if args.next().is_some() {
        return Err("too many arguments; expected one ELF path".into());
    }
...
fn validate_program_headers(elf: &Elf<'_>, file_len: usize) -> Result<(), Box<dyn Error>> {
    for phdr in &elf.program_headers {
        ...
        if end > file_len {
            return Err(format!(
                "program header (type {:#x}) extends past end of file: offset {start:#x}, size {:#x}, file len {file_len}",
