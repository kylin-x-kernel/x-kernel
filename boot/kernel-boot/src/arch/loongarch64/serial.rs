...
//! Minimal early-boot UART printing for LoongArch diagnostics.

use core::sync::atomic::{AtomicBool, AtomicUsize, Ordering};

use super::BOOT_DMW_UNCACHED_BASE;
...
static BOOT_CONSOLE_BASE: AtomicUsize = AtomicUsize::new(initial_boot_console_base());
static BOOT_CONSOLE_START_OF_LINE: AtomicBool = AtomicBool::new(true);

const fn initial_boot_console_base() -> usize {
    if kbuild_config::BOOT_CONSOLE_ADDR == 0 {
        0
    } else {
        BOOT_DMW_UNCACHED_BASE + kbuild_config::BOOT_CONSOLE_ADDR
    }
}
...
pub(crate) fn activate_linear_map() {
    ...
    // LoongArch keeps DMW windows active after paging turns on, and the early
    // boot console remains reliable there even before the runtime MMIO path is
    // fully established.
    BOOT_CONSOLE_BASE.store(BOOT_DMW_UNCACHED_BASE + paddr, Ordering::Relaxed);
}
...
#[unsafe(link_section = ".text.boot")]
pub(crate) fn boot_stage_banner(kernel_start_pa: usize) {
    if !is_enabled() {
        return;
    }
    write_str("loongarch _start kernel_pa=");
    write_hex(kernel_start_pa);
    write_str("\n");
}

#[unsafe(link_section = ".text.boot")]
pub(crate) fn boot_stage_checkpoint(step: usize) {
    if !is_enabled() {
        return;
    }
    write_str("checkpoint ");
    write_hex(step);
    write_str("\n");
}
