...
impl<H: Hal, T: Transport> RngInner<H, T> {
    fn try_new(mut transport: T, time: &'static dyn TimeOp) -> VirtioResult<Self> {
    ...
    fn request_entropy_timed(&mut self, dst: &mut [u8]) -> DriverResult<usize> {
        ...
        let deadline = self.time.monotonic_time() + REQUEST_TIMEOUT;
        while !self.queue.can_pop() {
            if self.time.monotonic_time() >= deadline {
...
impl<H: Hal, T: Transport> VirtIoRngDev<H, T> {
    ...
    pub fn try_new(transport: T, time: &'static dyn TimeOp) -> DriverResult<Self> {
        RngInner::try_new(transport, time)
            .map(|device| Self {
...
mod tests {
    ...
    impl TimeOp for MockTime {
        fn monotonic_time(&self) -> ktime_types::MonotonicInstant {
            ktime_types::MonotonicInstant::from_span_since_origin(TimeSpan::from_secs(1))
        }
    }

    static MOCK_TIME: MockTime = MockTime;

    #[def_test]
    fn test_virtio_rng_init_failure_handling() {
        let transport = MockTransport::new_with_type(DeviceType::EntropySource);
        let dev = VirtIoRngDev::<MockHal, MockTransport>::try_new(transport, &MOCK_TIME);
