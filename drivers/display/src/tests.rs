...
fn test_display_info_memory_alignment_validation() {
    ...
    for &base_addr in &alignments {
        ...
        assert_eq!(display.fb_base_vaddr % 0x1000, 0, "Address should be 4KB aligned");
        assert!(display.fb_base_vaddr >= 0x1000, "Address should be non-zero and reasonable");
        
        // Test framebuffer size is reasonable for given dimensions
        let expected_min_size = (display.width * display.height) as usize;
        assert!(display.fb_size >= expected_min_size, "Framebuffer size too small");
        
        // Common pixel formats: 1, 2, 3, 4 bytes per pixel
        let common_bpp = [1, 2, 3, 4];
        let is_valid_bpp = common_bpp.iter().any(|&bpp| {
            display.fb_size == (display.width * display.height * bpp) as usize
        });
        assert!(is_valid_bpp || display.fb_size >= expected_min_size * 4, 
...
fn test_framebuffer_creation_and_access_patterns() {
    ...
    for &size in &sizes {
        ...
        for (i, &byte) in buffer.iter().enumerate() {
            assert_eq!(byte, ((i + 0xAA) % 256) as u8, "Buffer corruption at index {}", i);
        }
    }
}
...
fn test_framebuffer_unsafe_operations_and_boundaries() {
    ...
    for &size in &test_sizes {
        ...
        for (i, &byte) in buffer.iter().enumerate() {
            assert_eq!(byte, (i % 255 + 1) as u8, "Memory corruption at index {}", i);
        }
        ...
        if size >= 4 {
            ...
            if size >= 8 {
                ...
                assert!(first_pattern != last_pattern || size == 8, 
