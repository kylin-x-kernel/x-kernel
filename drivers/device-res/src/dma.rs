...
use crate::{
    ResError, ResResult,
    provider::{dma_provider, try_dma_provider},
};
...
/// [`crate::set_dma_provider`]. The streaming / cache methods have default
/// no-op / `Unsupported` implementations so a host only overrides what its DMA
/// model needs (e.g. a bounce-buffer host leaves cache sync empty).
...
impl DmaCoherent {
    ...
    pub fn alloc(spec: DmaSpec) -> ResResult<Self> {
        let allocation = dma_provider()?.alloc_coherent(spec)?;
        Ok(Self {
...
impl Drop for DmaCoherent {
    fn drop(&mut self) {
        if let (Some(allocation), Some(provider)) = (self.allocation.take(), try_dma_provider()) {
...
impl<T> CoherentArray<T> {
    ...
    pub fn alloc(count: usize, extra_align: usize) -> ResResult<Self> {
        let align = core::mem::align_of::<T>().max(extra_align).max(1);
