...
use crate::{
    ResResult,
    provider::{mmio_provider, try_mmio_provider},
};
...
/// [`crate::set_mmio_provider`]. Runs in normal (non-interrupt) context during
...
impl Io {
    ...
    pub fn map(region: MmioRegion, name: &'static str) -> ResResult<Self> {
        let mapping = mmio_provider()?.map_mmio(region, name)?;
        Ok(Self {
...
impl Drop for Io {
    fn drop(&mut self) {
        if let (Some(mapping), Some(provider)) = (self.mapping.take(), try_mmio_provider()) {
