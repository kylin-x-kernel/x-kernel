...
pub mod tests_bcm2835sdhci {
    ...
    fn test_bcm2835_sdhci_interface_validation() {
        ...
        fn validate_sdhci_command(cmd: u8, arg: u32, has_data: bool, state: SdhciState) -> Result<(), SdhciError> {
            ...
            match cmd {
                0 => { // CMD0 (GO_IDLE_STATE) - should have arg=0
                    if arg != 0 {
                        return Err(SdhciError::InvalidArgument);
                    }
                }
                8 => { // CMD8 (SEND_IF_COND) - has specific arg format
        ...
        assert_eq!(validate_sdhci_command(64, 0, false, SdhciState::Idle), Err(SdhciError::InvalidCommand));
        assert_eq!(validate_sdhci_command(255, 0, false, SdhciState::Idle), Err(SdhciError::InvalidCommand));
        
        // Test invalid arguments
        assert_eq!(validate_sdhci_command(0, 123, false, SdhciState::Idle), Err(SdhciError::InvalidArgument));
        assert_eq!(validate_sdhci_command(8, 0x12345678, false, SdhciState::Idle), Err(SdhciError::InvalidArgument));
        
        // Test invalid states
        assert_eq!(validate_sdhci_command(0, 0, false, SdhciState::CommandInProgress), Err(SdhciError::InvalidCommand));
        assert_eq!(validate_sdhci_command(0, 0, false, SdhciState::DataTransfer), Err(SdhciError::InvalidCommand));
        assert_eq!(validate_sdhci_command(0, 0, false, SdhciState::Error), Err(SdhciError::InvalidCommand));
    }
    ...
    fn test_bcm2835_sdhci_timing_and_boundaries() {
        ...
        assert_eq!(calculate_timeout_cycles(1000, 50_000_000), Some(0x00FF_FFFF)); // Clamped to max
        ...
        assert_eq!(validate_transfer_size(512 * MAX_BLOCKS as usize, MAX_BLOCKS), Ok(MAX_BLOCKS));
        
        // Test invalid transfer sizes
        assert_eq!(validate_transfer_size(0, MAX_BLOCKS), Err(DriverError::InvalidInput));
        assert_eq!(validate_transfer_size(511, MAX_BLOCKS), Err(DriverError::InvalidInput));
        assert_eq!(validate_transfer_size(513, MAX_BLOCKS), Err(DriverError::InvalidInput));
        assert_eq!(validate_transfer_size(512 * (MAX_BLOCKS as usize + 1), MAX_BLOCKS), Err(DriverError::InvalidInput));
    }
}
