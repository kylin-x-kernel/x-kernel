...
mod tests_sdmmc {
    ...
    fn test_sdmmc_driver_boundary_conditions() {
        ...
        // Test invalid buffer sizes
        assert_eq!(simulate_block_operations(0, 511), Err(DriverError::InvalidInput));
        assert_eq!(simulate_block_operations(0, 513), Err(DriverError::InvalidInput));
        assert_eq!(simulate_block_operations(0, 1000), Err(DriverError::InvalidInput));
        
        // Test block ID overflow
        assert_eq!(simulate_block_operations(u32::MAX as u64 + 1, 512), Err(DriverError::Io));
        assert_eq!(simulate_block_operations(u64::MAX, 512), Err(DriverError::Io));
        
        // Test block range overflow
        assert_eq!(simulate_block_operations(u32::MAX as u64, 512), Err(DriverError::Io));
        assert_eq!(simulate_block_operations(u32::MAX as u64 - 1, 1024), Err(DriverError::Io));
        ...
        for (block_id, num_blocks, expected) in test_cases.iter() {
            ...
            assert_eq!(result, *expected, 
                      "Failed for block_id={}, num_blocks={}", block_id, num_blocks);
        }
