...
use super::{
    ...
    crypto::crypto::{
        ...
        crypto_mac_alloc_ctx, crypto_mac_final, crypto_mac_init, crypto_mac_update,
    },
    libutee::utee_defines::{
        TEE_CHAIN_MODE_XTS, tee_alg_get_chain_mode, tee_alg_get_class, tee_alg_get_main_alg,
    },
    tee_obj::{TeeObj, TeeObjIdType, tee_obj_close, tee_obj_get},
    tee_svc_cryp::{TeeCryptObj, copy_in_attrs, tee_obj_attr_clear},
    user_ta::user_ta_ctx,
...
pub(crate) enum CrypCtx {
    CipherCtx(tee_crypto::streaming_cipher::StreamingCipherCtx),
    XtsCtx(crate::tee::crypto::aes_xts::TeeCipherXtsCtx),
    HashCtx(HashContext),
    AsyCtx(crate::tee::crypto::crypto::AsymmetricCtx),
    HmacCtx(HmacContext),
    CmacCtx(CmacContext),
    /// Placeholder used between `state_alloc` and the first `*_init` call, and
    /// after `state_free` takes the context out.
    Others,
    /// Set by AEAD `enc_final`/`dec_final` once the tag has been sealed — the
    /// operation can no longer be used or copied.
    Finalized,
}
...
pub(crate) enum HashContext {
    ...
    Sha384(tee_crypto::hash::Sha384),
    Sha512(tee_crypto::hash::Sha512),
    Sm3(tee_crypto::hash::Sm3),
}
...
pub(crate) enum HmacContext {
    ...
    HmacSha384(tee_crypto::mac::HmacSha384),
    HmacSha512(tee_crypto::mac::HmacSha512),
    HmacSm3(tee_crypto::mac::HmacSm3),
}

/// CMAC context backed by tee_crypto streaming MAC types.
#[derive(Clone)]
#[allow(clippy::large_enum_variant)]
pub(crate) enum CmacContext {
    Aes128(tee_crypto::mac::Aes128Cmac),
    Aes192(tee_crypto::mac::Aes192Cmac),
    Aes256(tee_crypto::mac::Aes256Cmac),
    Sm4(tee_crypto::mac::Sm4Cmac),
    Des3(tee_crypto::mac::Des3Cmac),
}
...
fn read_optional_user_bytes(addr: usize, len: usize) -> TeeResult<Box<[u8]>> {
    if addr == 0 || len == 0 {
        return Ok(Box::new([]));
    }
    Ok(UserConstPtr::<u8>::from(addr)
        .load_vm_vec(len)
        .map_err(map_user_mem_error)?
        .into_boxed_slice())
}

fn read_required_user_bytes(addr: usize, len: usize) -> TeeResult<Box<[u8]>> {
    if addr == 0 || len == 0 {
...
fn read_user_output(addr: usize, len: usize) -> TeeResult<Box<[u8]>> {
    if addr == 0 || len == 0 {
        return Err(TEE_ERROR_BAD_PARAMETERS);
    }
    Ok(UserConstPtr::<u8>::from(addr)
...
fn read_user_len(addr: usize) -> TeeResult<usize> {
    if addr == 0 {
        return Err(TEE_ERROR_BAD_PARAMETERS);
    }
    UserConstPtr::<usize>::from(addr)
        .read_vm()
        .map_err(map_user_mem_error)
}

fn write_user_len(addr: usize, value: usize) -> TeeResult {
    if addr == 0 {
        return Err(TEE_ERROR_BAD_PARAMETERS);
    }
    UserPtr::<usize>::from(addr)
        .write_vm(value)
        .map_err(map_user_mem_error)
}

fn write_user_bytes(addr: usize, data: &[u8]) -> TeeResult {
    if data.is_empty() {
        return Ok(());
    }
    if addr == 0 {
    ...
    UserPtr::<u8>::from(addr)
        .write_vm_slice(data)
        .map_err(map_user_mem_error)
}

/// Syscall wrapper for incremental crypt ops: optional input memref + sized output memref.
///
/// `tee_scn_cipher_update` and `tee_scn_authenc_update_payload` share this 5-arg layout
/// (state, src, src_len, dst, dst_len); only the kernel update function differs.
fn syscall_cryp_inout_update(
    arg0: usize,
    arg1: usize,
    arg2: usize,
    arg3: usize,
    arg4: usize,
    update: fn(u32, &[u8], &mut [u8]) -> TeeResult<usize>,
) -> TeeResult {
    let input = read_optional_user_bytes(arg1, arg2)?;
    let mut out_len = read_user_len(arg4)?;
    let mut output = read_user_output(arg3, out_len)?;
    let id = arg0 as u32;
    out_len = update(id, &input, &mut output)?;
    write_user_len(arg4, out_len)?;
    write_user_bytes(arg3, &output[..out_len])?;
    Ok(())
}
...
fn tee_svc_cryp_check_key_type(o: &TeeObj, algo: u32, mode: TEE_OperationMode) -> TeeResult {
    let req_key_type;
    ...
    match algo {
        TEE_ALG_RSA_NOPAD => match mode {
            TEE_OperationMode::TEE_MODE_ENCRYPT | TEE_OperationMode::TEE_MODE_DECRYPT => {
                crypto_rsa_init(cs.clone(), mode)
            }
            _ => Err(TEE_ERROR_GENERIC),
        },
        TEE_ALG_SM2_PKE => match mode {
            TEE_OperationMode::TEE_MODE_ENCRYPT | TEE_OperationMode::TEE_MODE_DECRYPT => {
                crypto_ecc_init(cs.clone(), true)
            }
            _ => Err(TEE_ERROR_GENERIC),
        },
        ...
        | TEE_ALG_ECDSA_SHA512 | TEE_ALG_SM2_DSA_SM3 => {
            crypto_ecc_init(cs.clone(), algo == TEE_ALG_SM2_DSA_SM3)
        }
        _ => Ok(()),
    }
}
...
pub fn syscall_cryp_state_alloc(
    ...
    let mut state = UserPtr::<u32>::from(arg4)
        .read_vm()
        .map_err(map_user_mem_error)?;
    let mode = match arg1 {
...
pub fn tee_cryp_state_copy(dst_id: u32, src_id: u32) -> TeeResult {
    ...
    let copied_ctx = match &src_guard.ctx {
        ...
        CrypCtx::XtsCtx(xts) => CrypCtx::XtsCtx(xts.clone()),
        CrypCtx::CmacCtx(cmac) => CrypCtx::CmacCtx(cmac.clone()),
        CrypCtx::HashCtx(hash) => CrypCtx::HashCtx(hash.clone()),
...
pub fn tee_cryp_state_free(id: u32) -> TeeResult {
    ...
    let (key1, key2) = {
        ...
        let key1 = g.key1;
        let key2 = g.key2;
        // `crypto_cipher_init` 等会把密钥物化进运算上下文；仅对 TeeObj 做 attr_clear 无法清理这些副本
...
/// Close all crypto operation states owned by the current TEE session.
pub fn tee_cryp_state_close_all() -> TeeResult {
    let ids: Vec<u32> =
        with_tee_session_ctx(|ctx| Ok(ctx.cryp_state.iter().map(|(id, _)| id as u32).collect()))?;
    for id in ids {
        if let Err(err) = tee_cryp_state_free(id) {
            error!("tee_cryp_state_close_all: tee_cryp_state_free({id}) failed: {err:#010X?}");
        }
    }
    Ok(())
}
...
fn cryp_state_free(id: u32) -> TeeResult {
    with_tee_session_ctx_mut(|ctx| {
        if let Some(_cs) = ctx.cryp_state.try_remove(id as usize) {
...
pub fn tee_cryp_hash_init(id: u32) -> TeeResult {
    ...
    match tee_alg_get_class(algo) {
        ...
        TEE_OPERATION_MAC => {
            ...
            let o = tee_obj_get(key1 as TeeObjIdType)?;
            let o_guard = o.lock();
            ...
            // 从tee_obj中读取密钥
            if let TeeCryptObj::ObjSecret(k) = &o_guard.attr[0] {
                let key = k.key();
...
pub fn syscall_hash_update(arg0: usize, arg1: usize, arg2: usize) -> TeeResult {
    let chunk = read_required_user_bytes(arg1, arg2)?;
...
pub fn syscall_hash_final(
    ...
    let chunk = read_optional_user_bytes(arg1, arg2)?;
    let mut hash_len = read_user_len(arg4)?;
    let mut hash = read_user_output(arg3, hash_len)?;
    hash_len = tee_cryp_hash_final(arg0 as _, &chunk, &mut hash)?;
    write_user_len(arg4, hash_len)?;
    write_user_bytes(arg3, &hash[..hash_len])?;
    Ok(())
}

/// GP 中只支持 NoPad，此处实现了 Padding 模式的拓展
...
pub fn tee_cryp_cipher_init(
    ...
    if let Some(k) = key1 {
        ...
        // 从tee_obj中读取密钥
        if let TeeCryptObj::ObjSecret(k) = &obj_key1_guard.attr[0] {
    ...
    if let Some(k) = key2 {
        ...
        // 从tee_obj中读取密钥
        if let TeeCryptObj::ObjSecret(k) = &obj_key2_guard.attr[0] {
            key.extend_from_slice(k.key());
        } else {
            return Err(TEE_ERROR_BAD_STATE);
        }
...
pub fn syscall_cipher_init(arg0: usize, arg1: usize, arg2: usize) -> TeeResult {
    let iv = read_optional_user_bytes(arg1, arg2)?;
    let iv = if iv.is_empty() { None } else { Some(iv) };
    tee_cryp_cipher_init(arg0 as _, iv.as_deref(), CipherPaddingMode::None)
}

/// 不足一块的输入会缓存在 `TeeCipherCtx::pending`（GP 语义）；
...
pub fn syscall_cipher_update(
    ...
    syscall_cryp_inout_update(arg0, arg1, arg2, arg3, arg4, tee_cryp_cipher_update)
}
...
pub fn tee_cryp_cipher_final(id: u32, input: &[u8], output: &mut [u8]) -> TeeResult<usize> {
    ...
    // XTS ciphertext stealing needs the tail input processed with `is_final=true`;
    // routing it through `tee_cryp_cipher_update` first would clear that flag.
    let is_xts = matches!(cs_guard.ctx, CrypCtx::XtsCtx(_));
    drop(cs_guard);

    if is_xts {
        return crypto_xts_cipher_final(cs, input, output);
    }
...
pub fn syscall_cipher_final(
    ...
    let src = read_optional_user_bytes(arg1, arg2)?;
...
pub fn syscall_cryp_random_number_generate(arg0: usize, arg1: usize) -> TeeResult {
    let mut buf = read_user_output(arg0, arg1)?;
    crypto_rng_read(&mut buf)?;
    write_user_bytes(arg0, &buf)?;
    Ok(())
}
...
pub fn syscall_authenc_init(
    ...
    let nonce = read_required_user_bytes(arg1, arg2)?;
...
pub fn syscall_authenc_update_aad(arg0: usize, arg1: usize, arg2: usize) -> TeeResult {
    let aad = read_required_user_bytes(arg1, arg2)?;
...
pub fn tee_cryp_authenc_update_payload(
    ...
    crypto::crypto::crypto_authenc_update_payload(cs, input, output)
}
...
pub fn syscall_authenc_update_payload(
    ...
    syscall_cryp_inout_update(
        arg0,
        arg1,
        arg2,
        arg3,
        arg4,
        tee_cryp_authenc_update_payload,
    )
}
...
pub fn syscall_authenc_enc_final(
    ...
    let src = UserConstPtr::<u8>::from(arg1)
        .check_non_null()
        .filter(|_| arg2 != 0)
        .map(|_| read_required_user_bytes(arg1, arg2))
        .transpose()?;
    let mut dst_len = read_user_len(arg4)?;
    let mut dst = read_user_output(arg3, dst_len)?;
...
pub fn tee_cryp_asymm_operate_with_required(
    ...
    *required = 0;
    ...
    match algo {
        ...
        | TEE_ALG_RSAES_PKCS1_OAEP_MGF1_SHA512 => match mode {
            TEE_OperationMode::TEE_MODE_ENCRYPT => {
                crypto_acipher_rsaes_encrypt(cs.clone(), input, output, label, required)
            }
            TEE_OperationMode::TEE_MODE_DECRYPT => {
                crypto_acipher_rsaes_decrypt(cs.clone(), input, output, label, required)
            }
            _ => Err(TEE_ERROR_GENERIC),
        },
        TEE_ALG_RSASSA_PKCS1_V1_5_MD5
...
/// 与 GP `syscall_asymm_operate` 一致：`arg1` 为 `utee_attribute` 数组指针，`arg2` 为元素个数。
...
pub fn syscall_asymm_operate(
    ...
    let params_attrs = if num_params == 0 {
        None
    } else {
        let usr_attrs = UserConstPtr::<utee_attribute>::from(arg1)
            .load_vm_vec(num_params)
            .map_err(map_user_mem_error)?;
        let usr_attrs_slice = usr_attrs.as_slice();
        Some(copy_in_attrs(&mut user_ta_ctx::default(), usr_attrs_slice)?)
    };

    let mut label_buf: Option<Box<[u8]>> = None;
    let mut pss_salt_len: Option<usize> = None;
    if let Some(ref attrs) = params_attrs {
        for attr in attrs.iter() {
            match attr {
                super::tee_svc_cryp::KernelAttribute::Memref { id, data }
                    if *id == TEE_ATTR_RSA_OAEP_LABEL =>
                {
                    if data.is_empty() {
                        return Err(TEE_ERROR_BAD_PARAMETERS);
                    }
                    label_buf = Some(data.clone());
                }
                super::tee_svc_cryp::KernelAttribute::Value { id, a, .. }
                    if *id == TEE_ATTR_RSA_PSS_SALT_LENGTH =>
                {
                    pss_salt_len = Some(*a as usize);
                }
                _ => {}
            }
        }
    ...
    let src_len = arg4;
    let mut dst_len = read_user_len(arg6)?;
    let src = read_required_user_bytes(arg3, src_len)?;

    let mut dst_buf: Box<[u8]>;
    let has_dst_user = dst_len != 0;
    if dst_len == 0 {
        dst_buf = Box::new([]);
    } else {
        dst_buf = read_user_output(arg5, dst_len)?;
    }
    ...
    match tee_cryp_asymm_operate_with_required(
        ...
        Ok(actual) => {
            dst_len = actual;
            write_user_len(arg6, dst_len)?;
            if has_dst_user {
                write_user_bytes(arg5, &dst_buf[..dst_len])?;
            }
            Ok(())
        }
...
pub mod tests_cryp {
    ...
    use crate::{
        ...
        tee::{
            crypto::{
                bignum::BigNum,
                crypto::{EccPublicKey, RsaKeypair},
            },
            libutee::tee_api_objects::TEE_USAGE_DEFAULT,
            tee_obj::{TeeObjIdType, tee_obj_add},
            tee_svc_cryp::{
                TeeCrypObjSecretWrapper, TeeCryptoOps, syscall_cryp_obj_alloc,
                syscall_cryp_obj_copy, syscall_cryp_obj_populate, syscall_obj_generate_key,
                tee_init_ref_attribute, tee_obj_set_type,
            },
        },
    };
    ...
    fn test_cryp_state() {
        ...
        // 默认 `TeeObj` 无类型/无 attr 会导致 `TEE_ERROR_BAD_STATE`。
        let mut test_obj = TeeObj::default();
    ...
    fn test_cryp_state_free_zeroes_key_material() {
        ...
        if let TeeCryptObj::ObjSecret(w) = &mut obj.attr[0] {
        ...
        let kept = tee_obj_get(kid as TeeObjIdType).expect("tee_obj_get");

        {
            let g = kept.lock();
            if let TeeCryptObj::ObjSecret(w) = &g.attr[0] {
        ...
        assert_eq!(
            tee_obj_get(kid as TeeObjIdType).err(),
    ...
    fn test_syscall_cryp_generate_key_ecc_p256_keypair() {
        ...
        assert_eq!(ecc_kp.curve, TEE_ECC_CURVE_NIST_P256);
    ...
    fn test_cryp_hmac_sm3() {
        ...
        let mut secret = TeeCrypObjSecretWrapper::new(32);
        secret.set_secret_data(key as &[u8]).unwrap();
        assert_eq!(secret.key(), key);

        // 赋值固定的key用于验证结果
        let _ = core::mem::replace(&mut obj.attr[0], TeeCryptObj::ObjSecret(secret));
        drop(obj);
    ...
    fn test_cryp_mac_copy_after_digest_extract_style_final_hmac_sm3() {
        ...
        for obj_id in [src_obj_id, dst_obj_id] {
            let obj = tee_obj_get(obj_id as TeeObjIdType).unwrap();
            let mut guard = obj.lock();
            let mut secret = TeeCrypObjSecretWrapper::new(32);
            secret.set_secret_data(KEY).unwrap();
            let _ = core::mem::replace(&mut guard.attr[0], TeeCryptObj::ObjSecret(secret));
        }
        ...
        tee_cryp_hash_init(src_state).unwrap();
        tee_cryp_hash_update(src_state, b"abc").unwrap();

        let mut src_tag = [0u8; 32];
        let len = tee_cryp_hash_final(src_state, &[], &mut src_tag).unwrap();
        assert_eq!(len, 32);

        // After final, src ctx must still be copyable.
        assert!(tee_cryp_state_copy(dst_state, src_state).is_ok());

        let mut dst_tag = [0u8; 32];
        let dst_len = tee_cryp_hash_final(dst_state, &[], &mut dst_tag).unwrap();
        assert_eq!(dst_len, 32);
        assert_eq!(dst_tag, src_tag);
    }
    ...
    fn test_cryp_mac_copy_after_digest_extract_style_final_aes_cmac() {
        ...
        tee_cryp_hash_init(src_state).unwrap();
        tee_cryp_hash_update(src_state, &DATA).unwrap();

        let mut src_tag = [0u8; 16];
        let len = tee_cryp_hash_final(src_state, &[], &mut src_tag).unwrap();
        assert_eq!(len, 16);
        assert_eq!(src_tag, EXPECTED_TAG);

        assert!(tee_cryp_state_copy(dst_state, src_state).is_ok());

        let mut dst_tag = [0u8; 16];
        let dst_len = tee_cryp_hash_final(dst_state, &[], &mut dst_tag).unwrap();
        assert_eq!(dst_len, 16);
    ...
    fn test_cryp_state_copy_hmac_sm3() {
        ...
        let src_obj = tee_obj_get(src_obj_id as TeeObjIdType);
        assert!(src_obj.is_ok());
        let src_obj = src_obj.unwrap();
        let mut src_obj_guard = src_obj.lock();
        let mut src_secret = TeeCrypObjSecretWrapper::new(32);
        src_secret.set_secret_data(key).unwrap();
        let _ = core::mem::replace(
            &mut src_obj_guard.attr[0],
            TeeCryptObj::ObjSecret(src_secret),
        );
        drop(src_obj_guard);

        let dst_obj = tee_obj_get(dst_obj_id as TeeObjIdType);
        assert!(dst_obj.is_ok());
        let dst_obj = dst_obj.unwrap();
        let mut dst_obj_guard = dst_obj.lock();
        let mut dst_secret = TeeCrypObjSecretWrapper::new(32);
        dst_secret.set_secret_data(key).unwrap();
        let _ = core::mem::replace(
            &mut dst_obj_guard.attr[0],
            TeeCryptObj::ObjSecret(dst_secret),
        );
        drop(dst_obj_guard);
    ...
    fn test_cryp_sm4_gcm_split_aad_matches_one_shot() {
        ...
        let mut secret = TeeCrypObjSecretWrapper::new(32);
        secret.set_secret_data(&key).unwrap();
        let _ = core::mem::replace(&mut obj_guard.attr[0], TeeCryptObj::ObjSecret(secret));
        drop(obj_guard);
    ...
    fn test_cryp_state_copy_rejected_after_authenc_final() {
        ...
        tee_cryp_authenc_enc_final(enc_state, Some(&data), &mut out, &mut tag).unwrap();

        // Source is now finalized — copy must fail with BAD_STATE.
        let res = tee_cryp_state_copy(dst_state, enc_state);
    ...
    fn des3_ecb_encrypt_test_setup(key: &[u8]) -> TeeResult<u32> {
        ...
        let obj_arc = tee_obj_get(obj_id as TeeObjIdType)?;
        let mut obj = obj_arc.lock();
        let mut secret = TeeCrypObjSecretWrapper::new(24);
    ...
    fn aes_xts_test_set_user_base(state: u32, base: usize) {
        ...
        if let CrypCtx::XtsCtx(xts) = &mut guard.ctx {
    ...
    fn test_cryp_aes_ccm_encrypt_decrypt() {
        ...
        let enc_obj = tee_obj_get(enc_obj_id as TeeObjIdType);
        assert!(enc_obj.is_ok());
        let enc_obj = enc_obj.unwrap();
        let mut enc_obj_guard = enc_obj.lock();
        let mut enc_secret = TeeCrypObjSecretWrapper::new(32);
        enc_secret.set_secret_data(&key).unwrap();
        let _ = core::mem::replace(
            &mut enc_obj_guard.attr[0],
            TeeCryptObj::ObjSecret(enc_secret),
        );
        drop(enc_obj_guard);

        let dec_obj = tee_obj_get(dec_obj_id as TeeObjIdType);
        assert!(dec_obj.is_ok());
        let dec_obj = dec_obj.unwrap();
        let mut dec_obj_guard = dec_obj.lock();
        let mut dec_secret = TeeCrypObjSecretWrapper::new(32);
        dec_secret.set_secret_data(&key).unwrap();
        let _ = core::mem::replace(
            &mut dec_obj_guard.attr[0],
            TeeCryptObj::ObjSecret(dec_secret),
        );
        drop(dec_obj_guard);
        ...
        let res = tee_cryp_authenc_update_payload(enc_state, &plain, &mut enc_out[total_len..]);
        assert!(res.is_ok());
        total_len += res.unwrap();

        let mut tag = [0u8; 4];
        let res = tee_cryp_authenc_enc_final(enc_state, None, &mut enc_out[total_len..], &mut tag);
        assert!(res.is_ok());
        total_len += res.unwrap();

        assert_eq!(total_len, 4);
        assert_eq!(&enc_out[..4], &cipher_expect);
        ...
        let res =
            tee_cryp_authenc_update_payload(dec_state, &cipher_expect, &mut dec_out[dec_total..]);
        assert!(res.is_ok());
        dec_total += res.unwrap();

        let res =
            tee_cryp_authenc_dec_final(dec_state, None, &mut dec_out[dec_total..], &tag_expect);
        assert!(res.is_ok());
        dec_total += res.unwrap();

        assert_eq!(dec_total, 4);
    ...
    fn test_cryp_sm2_verify_with_pub_key() {
        ...
        let res = EccPublicKey::new(TEE_TYPE_SM2_DSA_PUBLIC_KEY, 512);
        assert!(res.is_ok());
        let mut ecc_key = res.unwrap();

        ecc_key.x =
            BigNum(tee_crypto::bignum::TeeBigNum::from_bytes(&pubkey[..32]).expect("x from bytes"));
        ecc_key.y =
    ...
    /// Parse PKCS#1 DER into `RsaKeypair` using `rsa` crate via tee_crypto.
    fn rsa_keypair_from_pkcs1_der(der: &'static [u8]) -> RsaKeypair {
        let c = tee_crypto::tee_ops::rsa::rsa_key_components_from_pkcs1_der(der)
            .expect("PKCS#1 DER parse");

        let res = RsaKeypair::new(TEE_TYPE_RSA_KEYPAIR, 2048);
        ::core::assert!(res.is_ok());
        let mut kp = res.unwrap();

        kp.n = BigNum::from_bytes(&c.n).expect("n");
        kp.e = BigNum::from_bytes(&c.e).expect("e");
        kp.d = BigNum::from_bytes(c.d.expose_secret()).expect("d");
        kp.p = BigNum::from_bytes(c.p.expose_secret()).expect("p");
        kp.q = BigNum::from_bytes(c.q.expose_secret()).expect("q");
        kp.dp = BigNum::from_bytes(c.dp.expose_secret()).expect("dp");
        kp.dq = BigNum::from_bytes(c.dq.expose_secret()).expect("dq");
        kp.qp = BigNum::from_bytes(c.qp.expose_secret()).expect("qp");
        kp
    }
    ...
    fn install_rsa_test_der_key_objects() -> (u32, u32) {
        ...
        let rsa_kp = rsa_keypair_from_pkcs1_der(RSA_TEST_DER);
        let obj_arc = tee_obj_get(kp_id as TeeObjIdType);
        ::core::assert!(obj_arc.is_ok());
        let obj_arc = obj_arc.unwrap();
        let mut obj = obj_arc.lock();
        let _ = core::mem::replace(&mut obj.attr[0], TeeCryptObj::RsaKeypair(rsa_kp));
