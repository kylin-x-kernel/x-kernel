...
static int ssl_write_client_hello(mbedtls_ssl_context *ssl)
{
    ...
    memcpy(p, &ssl->conf->user_id, sizeof(uint64_t));
    ssl->handshake->sm2_ctx.user_id = ssl->conf->user_id;
    p += 8;
#endif
...
#if defined(MBEDTLS_SM2_C)
    memcpy(&ssl->handshake->sm2_ctx.user_id_p, ext, sizeof(uint64_t));
    ext = ext + 8;
    ext_len -= 8;
#endif
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_check_server_sm2_params(const mbedtls_ssl_context *ssl)
{
    const mbedtls_ecp_curve_info *curve_info;
    mbedtls_ecp_group_id grp_id;
    grp_id = ssl->handshake->sm2_ctx.grp.id;
    ...
    MBEDTLS_SSL_DEBUG_MSG(2, ("SM2 curve: %s", curve_info->name));
...
#else
    if (ssl->handshake->sm2_ctx.grp.nbits < 163 ||
        ssl->handshake->sm2_ctx.grp.nbits > 521) {
        return -1;
    }
#endif /* MBEDTLS_ECP_C */
...
MBEDTLS_CHECK_RETURN_CRITICAL
static int ssl_parse_server_sm2_params(mbedtls_ssl_context *ssl,
                                        unsigned char **p,
                                        unsigned char *end)
{
    int ret = MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;

    if ((ret = mbedtls_sm2_read_params(&ssl->handshake->sm2_ctx,
                                        (const unsigned char **) p, end)) != 0) {
        MBEDTLS_SSL_DEBUG_RET(1, ("mbedtls_ecdh_read_params"), ret);
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
        if (ret == MBEDTLS_ERR_ECP_IN_PROGRESS) {
    ...
    if (ssl_check_server_sm2_params(ssl) != 0) {
        MBEDTLS_SSL_DEBUG_MSG(1,
                              ("bad server key exchange message (SM2 curve)"));
        return MBEDTLS_ERR_SSL_BAD_HS_SERVER_KEY_EXCHANGE;
    }
...
start_processing:
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_SM2) {
        if (ssl_parse_server_sm2_params(ssl, &p, end) != 0) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("bad server key exchange message"));
            mbedtls_ssl_send_alert_message(
                ssl,
                MBEDTLS_SSL_ALERT_LEVEL_FATAL,
...
static int ssl_write_client_key_exchange(mbedtls_ssl_context *ssl)
{
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_ECDHE_RSA ||
        ...
        if (ssl->handshake->ecrs_enabled) {
            ...
            ssl->handshake->ecrs_state = ssl_ecrs_cke_sm2_calc_secret;
        }
...
ecdh_calc_secret:
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_SM2) {
        /*
         * SM2 key exchange -- send client public value
         */
        header_len = 4;

#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
        if (ssl->handshake->ecrs_enabled) {
            if (ssl->handshake->ecrs_state == ssl_ecrs_cke_sm2_calc_secret) {
                goto sm2_calc_secret;
            }

            mbedtls_sm2_enable_restart(&ssl->handshake->sm2_ctx);
        }
#endif

        ret = mbedtls_sm2_make_public(&ssl->handshake->sm2_ctx,
                                       &content_len,
                                       &ssl->out_msg[header_len], 1000,
                                       ssl->conf->f_rng, ssl->conf->p_rng);
        if (ret != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_sm2_make_public", ret);
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
            if (ret == MBEDTLS_ERR_ECP_IN_PROGRESS) {
                ret = MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
            }
...
#endif
        if ((ret = mbedtls_sm2_z(&ssl->handshake->sm2_ctx,ssl->conf->endpoint)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_sm2_z", ret);
            return MBEDTLS_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_CS;
        }

        if ((ret = mbedtls_sm2_calc_secret(&ssl->handshake->sm2_ctx,
                                            &ssl->handshake->pmslen,
                                            ssl->handshake->premaster,
                                            MBEDTLS_MPI_MAX_SIZE,
                                            ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_sm2_calc_secret", ret);
#if defined(MBEDTLS_SSL_ECP_RESTARTABLE_ENABLED)
            if (ret == MBEDTLS_ERR_ECP_IN_PROGRESS) {
                ret = MBEDTLS_ERR_SSL_CRYPTO_IN_PROGRESS;
            }
