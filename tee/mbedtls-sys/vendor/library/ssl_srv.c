...
read_record_header:
    ...
    memcpy(&ssl->handshake->sm2_ctx.user_id_p, buf + 2, sizeof(uint64_t));
    buf = buf + 8;
    msg_len -= 8;
#endif
...
#if defined(MBEDTLS_SM2_C)
    // 添加 user_id 字段
    memcpy(p + 2 + ext_len, &ssl->conf->user_id, sizeof(uint64_t));
...
curve_matching_done:
     ...
     */
#if defined(MBEDTLS_KEY_EXCHANGE_SOME_ECDHE_ENABLED)
    if (mbedtls_ssl_ciphersuite_uses_sm2(ciphersuite_info)) {
        /*
         * Ephemeral ECDH parameters:
         *
         * struct {
        ...
        for (gid = ssl->conf->curve_list; *gid != MBEDTLS_ECP_DP_NONE; gid++) {
            for (curve = ssl->handshake->curves; *curve != NULL; curve++) {
                if ((*curve)->grp_id == *gid) {
                    goto sm2_curve_matching_done;
                }
            }
        }

sm2_curve_matching_done:
        if (curve == NULL || *curve == NULL) {
            MBEDTLS_SSL_DEBUG_MSG(1, ("no matching curve for SM2"));
            return MBEDTLS_ERR_SSL_NO_CIPHER_CHOSEN;
        }

        MBEDTLS_SSL_DEBUG_MSG(2, ("SM2 curve: %s", (*curve)->name));

        if ((ret = mbedtls_sm2_setup(&ssl->handshake->sm2_ctx,
                                      (*curve)->grp_id)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_ecp_group_load", ret);
            return ret;
        }

        if ((ret = mbedtls_sm2_make_params(
                 &ssl->handshake->sm2_ctx, &len,
                 ssl->out_msg + ssl->out_msglen,
                 MBEDTLS_SSL_OUT_CONTENT_LEN - ssl->out_msglen,
                 ssl->conf->f_rng, ssl->conf->p_rng)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_sm2_make_params", ret);
            return ret;
        }
...
static int ssl_parse_client_key_exchange(mbedtls_ssl_context *ssl)
{
    ...
    if (ciphersuite_info->key_exchange == MBEDTLS_KEY_EXCHANGE_SM2) {
        if ((ret = mbedtls_sm2_read_public(&ssl->handshake->sm2_ctx,
                                            p, end - p)) != 0) {
            MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_sm2_read_public", ret);
            return MBEDTLS_ERR_SSL_BAD_HS_CLIENT_KEY_EXCHANGE_RP;
        }
