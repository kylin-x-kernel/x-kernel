...
static void ssl_calc_verify_tls_sm3(const mbedtls_ssl_context *, unsigned char *, size_t *);
static int ssl_calc_finished_tls_sm3(mbedtls_ssl_context *, unsigned char *, int);
#endif
...
#endif
#if defined(MBEDTLS_SM3_C)
    if (tls_prf == tls_prf_sm3) {
        return MBEDTLS_SSL_TLS_PRF_SM3;
    } else
#endif
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
...
#if defined(MBEDTLS_SM3_C)
void ssl_calc_verify_tls_sm3(const mbedtls_ssl_context *ssl,
                                unsigned char *hash,
                                size_t *hlen)
{
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    size_t hash_size;
    psa_status_t status;
    psa_hash_operation_t sm3_psa = psa_hash_operation_init();

    MBEDTLS_SSL_DEBUG_MSG(2, ("=> PSA calc verify sm3"));
    status = psa_hash_clone(&ssl->handshake->fin_sm3_psa, &sm3_psa);
    if (status != PSA_SUCCESS) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("PSA hash clone failed"));
        return;
    }

    status = psa_hash_finish(&sm3_psa, hash, 32, &hash_size);
    if (status != PSA_SUCCESS) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("PSA hash finish failed"));
        return;
    }
    ...
    *hlen = 32;
    ...
    mbedtls_sm3_finish_ret(&sm3, hash);
...
int mbedtls_ssl_write_certificate(mbedtls_ssl_context *ssl)
{
    ...
    /* 获取长期密钥 */
    if ((ret = mbedtls_sm2_set_key(&ssl->handshake->sm2_ctx,
        (mbedtls_sm2_context *)mbedtls_ssl_own_key(ssl)->pk_ctx)) != 0){
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_sm2_set_key", ret);
        return ret;
    }
...
crt_verify:
    ...
    if((ret = mbedtls_sm2_set_peerkey(&ssl->handshake->sm2_ctx,
        (mbedtls_sm2_context *)peer_pk->pk_ctx)) != 0){
        MBEDTLS_SSL_DEBUG_RET(1, "mbedtls_sm2_set_peerkey", ret);
        return ret;
    }
...
#if defined(MBEDTLS_SM3_C)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    psa_hash_abort(&ssl->handshake->fin_sm3_psa);
    psa_hash_setup(&ssl->handshake->fin_sm3_psa, PSA_ALG_SHA_256);
#else
    mbedtls_sm3_starts_ret(&ssl->handshake->fin_sm3);
#endif
#endif
#if defined(MBEDTLS_SHA512_C) && !defined(MBEDTLS_SHA512_NO_SHA384)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
...
#if defined(MBEDTLS_SM3_C)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    psa_hash_update(&ssl->handshake->fin_sm3_psa, buf, len);
#else
    mbedtls_sm3_update_ret(&ssl->handshake->fin_sm3, buf, len);
#endif
#endif
#if defined(MBEDTLS_SHA512_C) && !defined(MBEDTLS_SHA512_NO_SHA384)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
...
#if defined(MBEDTLS_SM3_C)
static int ssl_calc_finished_tls_sm3(
    mbedtls_ssl_context *ssl, unsigned char *buf, int from)
{
    int len = 12;
    const char *sender;
...
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    sm3_psa = psa_hash_operation_init();

    MBEDTLS_SSL_DEBUG_MSG(2, ("=> calc PSA finished tls sm3"));
    ...
    status = psa_hash_finish(&sm3_psa, padbuf, sizeof(padbuf), &hash_size);
    if (status != PSA_SUCCESS) {
        MBEDTLS_SSL_DEBUG_MSG(2, ("PSA hash finish failed"));
        return mbedtls_ssl_md_error_from_psa(status);
    }
    ...
    mbedtls_sm3_finish_ret(&sm3, padbuf);
    mbedtls_sm3_free(&sm3);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
...
#if defined(MBEDTLS_SM3_C)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    handshake->fin_sm3_psa = psa_hash_operation_init();
    psa_hash_setup(&handshake->fin_sm3_psa, PSA_ALG_SHA_256);
#else
    mbedtls_sm3_init(&handshake->fin_sm3);
    mbedtls_sm3_starts_ret(&handshake->fin_sm3);
#endif
#endif
#if defined(MBEDTLS_SHA512_C) && !defined(MBEDTLS_SHA512_NO_SHA384)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
...
void mbedtls_ssl_conf_user_id(mbedtls_ssl_config *conf, uint64_t id)
{
    conf->user_id = id;
}
...
#if defined(MBEDTLS_SM3_C)
#if defined(MBEDTLS_USE_PSA_CRYPTO)
    psa_hash_abort(&handshake->fin_sm3_psa);
#else
                                ...
                                int endpoint, int transport, int preset)
{
    ...
    mbedtls_ssl_conf_user_id(conf, 0);
                                  ...
                                  mbedtls_md_type_t md_alg)
{
    switch (sig_alg) {
        ...
        case MBEDTLS_PK_SM2:
            if (set->sm2 == MBEDTLS_MD_NONE) {
                set->sm2 = MBEDTLS_MD_SM3;
            }
            break;
