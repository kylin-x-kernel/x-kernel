...
#error "This SM2 implementation needs 64 bytes to store the public key."
#endif
#if DICE_PRIVATE_KEY_BUFFER_SIZE != 32
#error "SM2 needs 32 bytes for the private key."
#endif
#if DICE_SIGNATURE_BUFFER_SIZE != 64
#error "SM2 needs 64 bytes to store the signature."
#endif

#define DICE_PROFILE_NAME "opendice.example.sm2"
...
DiceResult DiceGetKeyParam(void* context_not_used,
  ...
  key_param->cose_key_algorithm = kCoseAlgSM2;
  key_param->cose_key_curve = kCoseCrvSM2;
...
DiceResult DiceKeypairFromSeed(
  ...
  (void)context_not_used;
  (void)principal_not_used;
  if (1 == SM2KeypairFromSeed(public_key, private_key, seed)) {
    return kDiceResultOk;
  }
...
DiceResult DiceSign(void* context_not_used, const uint8_t* message,
  ...
  (void)context_not_used;
  if (1 == SM2Sign(signature, message, message_size, private_key)) {
    return kDiceResultOk;
  }
  return kDiceResultPlatformError;
}
...
DiceResult DiceVerify(void* context_not_used, const uint8_t* message,
  ...
  (void)context_not_used;
  if (1 == SM2Verify(message, message_size, signature, public_key)) {
    return kDiceResultOk;
  }
  return kDiceResultPlatformError;
}
