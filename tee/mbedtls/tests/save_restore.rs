...
use mbedtls::{
    cipher,
    cipher::{
        Authenticated, Cipher, Decryption, Encryption, Fresh, Traditional,
        raw::{CipherId, CipherMode, CipherPadding},
    },
};
...
fn save_restore_aes_cbc_dec_pkcs7() {
    ...
    // For decryption with padding, mbedtls keeps the last block until we do
    // `finish()`
    let (len1, cipher_d1) = cipher_k.update(&ct[0..16], &mut pt[0..32]).unwrap();
    assert_eq!(len1, 0);
...
fn save_restore_aes_gcm_enc() {
    ...
    let cipher = cipher::Cipher::<Encryption, Authenticated, Fresh>::new(
        CipherId::Aes,
        CipherMode::GCM,
        128,
    )
    ...
    let cipher_r =
        de::from_slice::<Cipher<Encryption, Authenticated, _>>(saved.as_slice()).unwrap();
...
fn save_restore_aes_gcm_dec() {
    ...
    let cipher = cipher::Cipher::<Decryption, Authenticated, Fresh>::new(
        CipherId::Aes,
        CipherMode::GCM,
        128,
    )
    ...
    let cipher_r =
        de::from_slice::<Cipher<Decryption, Authenticated, _>>(saved.as_slice()).unwrap();
