...
fn client(conn: TcpStream, test: Test) -> TlsResult<()> {
    ...
    let verify_callback =
        move |_crt: &Certificate, _depth: i32, verify_flags: &mut VerifyError| match verify_test {
    ...
    match (
        test,
        ctx.establish(conn, None).err().expect("should have failed"),
    ) {
...
#[cfg(unix)]
mod test {
    use crate::support::{net::create_tcp_pair, thread_spawn_named};
    ...
    fn callback_error() {
        ...
        let c = thread_spawn_named("client", move || {
            super::client(c, super::Test::CallbackError).unwrap()
        });
        let s = thread_spawn_named("server", move || super::server(s).unwrap());
        c.join().unwrap();
        s.join().unwrap();
