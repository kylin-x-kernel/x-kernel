...
use mbedtls::{
    ...
    rng::CtrDrbg,
    ssl::{
        Config, Context, CookieContext, Io, Version,
        config::{Endpoint, Preset, Transport},
        context::Timer,
        io::{ConnectedUdpSocket, IoCallback},
    },
...
fn client<C: IoCallback<T> + TransportType, T>(
    ...
    assert_eq!(
        <C as TransportType>::send(&mut ctx, buf.as_bytes()).unwrap(),
        buf.len()
    );
    let mut buf = [0u8; 13 + 4 + 1];
    assert_eq!(
        <C as TransportType>::recv(&mut ctx, &mut buf).unwrap(),
        buf.len()
    );
...
mod test {
    ...
    fn client_server_test() {
        ...
        use mbedtls::ssl::{Version, io::ConnectedUdpSocket};
    ...
    fn write_large_buffer_should_ok() {
        ...
        let expected_data: Vec<u8> = std::iter::repeat_with(rand::random)
            .take(buffer_size)
            .collect();
