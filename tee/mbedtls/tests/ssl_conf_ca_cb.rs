...
use mbedtls::{
    ...
    ssl::{
        Config, Context,
        config::{CaCallback, Endpoint, Preset, Transport},
    },
    x509::Certificate,
};
...
mod test {
    ...
    use super::*;
    use crate::support::{keys, net::create_tcp_pair, thread_spawn_named};

    // This callback should accept any valid self-signed certificate
    fn self_signed_ca_callback(
        child: &MbedtlsList<Certificate>,
    ) -> TlsResult<MbedtlsList<Certificate>> {
        Ok(child.clone())
    }
    ...
    fn callback_self_signed() {
        let (c, s) = create_tcp_pair().unwrap();
        let c = thread_spawn_named("client", move || {
            super::client(c, self_signed_ca_callback).unwrap()
        });
        let s = thread_spawn_named("server", move || {
    ...
    fn callback_self_signed_invalid_sig() {
        ...
        let s = thread_spawn_named("server", move || {
            super::server(
                s,
                keys::PEM_SELF_SIGNED_CERT_INVALID_SIG,
                keys::PEM_SELF_SIGNED_KEY,
            )
            .unwrap()
        });
        c.join().unwrap();
