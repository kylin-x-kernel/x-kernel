...
// This is a test key generated with the pk::generate_ec_sm2p256r1 test.
const TEST_KEY_PEM_SM2: &'static str = "-----BEGIN EC PRIVATE KEY-----
MHcCAQEEIAI9mk2ehOJTlZ6aIY+9/G2NAMfzLzVetyCtRoH8743coAoGCCqBHM9V
AYItoUQDQgAEztuDIQLD4VKkrbJIxUdp90nruIbVqFAfFSsn/nrmaRLEvoLjTdFD
UhT/zoClRnvHVrIbPLPqK1njPHdRIG+7ng==
-----END EC PRIVATE KEY-----
\0";
...
#[test]
fn sign_verify_sm2_1() {
    let mut k = Pk::generate_sm2(&mut test_rng(), EcGroupId::SM2P256R1).unwrap();
    let data = b"SIGNATURE TEST SIGNATURE TEST SI";
    let mut signature1 = [0u8; ECDSA_MAX_LEN];
    let mut signature2 = [0u8; ECDSA_MAX_LEN];
    let mut rng = test_rng();

    let len = k.sign(SM3, data, &mut signature1, &mut rng).unwrap();
    k.verify(SM3, data, &signature1[0..len]).unwrap();

    let len = k.sign(SM3, data, &mut signature2, &mut rng).unwrap();
    k.verify(SM3, data, &signature2[0..len]).unwrap();

    assert!(&signature1[..] != &signature2[..]);
}

#[test]
fn sign_verify_sm2_2() {
    let mut k = Pk::from_private_key(TEST_KEY_PEM_SM2.as_bytes(), None).unwrap();
    let data = b"SIGNATURE TEST SIGNATURE TEST SI";
    let mut signature1 = [0u8; ECDSA_MAX_LEN];
    let mut signature2 = [0u8; ECDSA_MAX_LEN];
    let mut rng = test_rng();
