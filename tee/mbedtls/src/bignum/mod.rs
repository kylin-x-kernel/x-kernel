...
use core::{
    cmp::Ordering,
    fmt::{Binary, Debug, Display, Formatter, Octal, Result as FmtResult, UpperHex},
    ops::{
...
impl Mpi {
    ...
    pub fn mpi_grow(&mut self, nblimbs: usize) -> Result<()> {
        unsafe {
            mpi_grow(&mut self.inner, nblimbs);
        }
        Ok(())
    }
    ...
    pub fn clear(&mut self) {
        unsafe {
            let size = mpi_size(&self.inner);
            if size > 0 {
                // Clear the memory
                ::core::ptr::write_bytes(self.inner.p as *mut u8, 0, size);
            }
        }
    }
...
mod tests {
    ...
    fn test_less_than_const_time() {
        ...
        assert_eq!(
            mpi2.less_than_const_time(&mpi3),
            Err(codes::MpiBadInputData.into())
        );
    }
