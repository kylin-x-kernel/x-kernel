...
use crate::{
    ...
    private::UnsafeFrom,
    rng::RngCallback,
    ssl::{context::HandshakeContext, cookie::CookieCallback, ticket::TicketCallback},
    x509::{self, Certificate, Crl, Profile, VerifyCallback},
};
...
impl Config {
    ...
    pub fn set_user_id(&mut self, n: u64) {
        unsafe { ssl_conf_user_id(self.into(), n) }
        self.user_id = n;
    }
        ...
        F: CaCallback + 'static,
    {
            ...
            F: CaCallback + 'static,
        {
            ...
            let cb = &mut *(closure as *mut F);
            let crt: &MbedtlsList<Certificate> =
                UnsafeFrom::from(&child as *const *const x509_crt).expect("valid certificate");
            match cb(&crt) {
                Ok(list) => {
