...
use crate::{
    ...
    private::UnsafeFrom,
    ssl::{
        config::{AuthMode, Config, Version},
        io::IoCallbackUnsafe,
    },
    x509::{Certificate, Crl, VerifyError},
};
...
impl<T> Context<T> {
    ...
    fn set_hostname(&mut self, hostname: Option<&str>) -> Result<()> {
        if let Some(s) = hostname {
            let cstr =
                ::std::ffi::CString::new(s).map_err(|_| Error::from(codes::SslBadInputData))?;
            unsafe {
...
mod tests {
    ...
    use crate::{
        ssl::context::{Context, HandshakeContext},
        tests::TestTrait,
    };
