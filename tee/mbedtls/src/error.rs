...
use core::{convert::Infallible, fmt, ops::BitOr, str::Utf8Error};
#[cfg(feature = "std")]
...
impl Error {
    ...
    pub fn to_int(&self) -> c_int {
        match self {
            ...
            &Error::HighAndLowLevel(hl_error, ll_error) => {
                c_int::from(hl_error) + c_int::from(ll_error)
            }
...
#[cfg(test)]
mod tests {
    use super::{Error, HiError, LoError, codes};
    ...
    fn test_common_error_operations() {
        ...
        assert_eq!(
            combined_error.to_int(),
            hi_only_error.to_int() + lo_only_error.to_int()
        );
        assert_eq!(
            codes::CipherAllocFailed | codes::AesBadInputData,
            combined_error
        );
        assert_eq!(
            codes::AesBadInputData | codes::CipherAllocFailed,
            combined_error
        );
    }
    ...
    fn test_error_display() {
        ...
        assert_eq!(
            format!("{}", hi_only_error),
            "mbedTLS error HiError :: CipherAllocFailed"
        );
        assert_eq!(
            format!("{}", lo_only_error),
            "mbedTLS error LoError :: AesBadInputData"
        );
        assert_eq!(
            format!("{}", combined_error),
            "(mbedTLS error HiError :: CipherAllocFailed, mbedTLS error LoError :: \
             AesBadInputData)"
        );
    }
    ...
    fn test_error_from_int() {
        ...
        assert_eq!(
            Error::from(-0x80),
            Error::HighLevel(HiError::Unknown(-0x80))
        );
        assert_eq!(
            Error::from(-0x81),
        ...
        assert_eq!(
            Error::from(-24960),
            Error::HighLevel(HiError::CipherAllocFailed)
        );
