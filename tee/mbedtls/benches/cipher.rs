...
use mbedtls::cipher::{Authenticated, Cipher, Traditional, raw};
...
fn aes_decrypt_auth(
    ...
    let cipher =
        Cipher::<_, Authenticated, _>::new(raw::CipherId::Aes, cipher_mode, (key.len() * 8) as _)
            .unwrap();
    let cipher = cipher.set_key_iv(key, iv).unwrap();
    cipher
        .decrypt_auth(ad, cipher_and_tag, plain_out, tag_len)
        .unwrap();
}
...
fn aes_encrypt_auth(
    ...
    let cipher = cipher.set_key_iv(key, iv).unwrap();
    cipher
        .encrypt_auth(ad, plain_text, cipher_and_tag_out, tag_len)
        .unwrap();
}
...
fn aes_decrypt(
    ...
    let cipher =
        Cipher::<_, Traditional, _>::new(raw::CipherId::Aes, cipher_mode, (key.len() * 8) as _)
            .unwrap();
    let cipher = cipher.set_key_iv(key, iv).unwrap();
