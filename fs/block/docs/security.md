...
- `FileSystemType::mount_bdev` 进入解析不可信磁盘元数据的具体文件系统。
...
- 最终镜像必须恰好提供一个 `FileSystemType` 实现。

## 线程安全

`FileSystemType` 没有运行时可变注册状态。`SeekableDisk` 不提供内部并发控制；具体
...
| T-02 | boot 与 Kconfig 选择不同文件系统实现 | 中 | 维护重复运行时分支 | boot 只调用 exactly-one `FileSystemType`；Kconfig/Cargo 决定 provider |
...
当前 Kconfig 每个镜像只选择一个 block root filesystem type，因此接口是 exactly-one。
同镜像动态注册多个 block filesystem type 尚未实现。

## 审计清单

- 新 root filesystem 是否只提供 `FileSystemType`，而未在 boot 增加名称分支？
- provider 是否完整校验不可信磁盘输入并传播 mount 错误？
- `SeekableDisk` 用户是否在持久化边界显式 flush？
- 是否意外启用了两个 `FileSystemType` provider？
