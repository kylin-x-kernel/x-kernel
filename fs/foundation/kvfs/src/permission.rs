...
pub fn check_permission(
    ...
    if permission.is_empty() || has_all_requested_bits(metadata.mode, permission) {
        return Ok(());
    }
    ...
    let is_allowed = if credentials.uid() == metadata.uid {
        owner_allows(metadata.mode, permission)
    } else if credentials.has_group(metadata.gid) {
        group_allows(metadata.mode, permission)
    } else {
        other_allows(metadata.mode, permission)
    };
...
mod tests {
    ...
    use crate::{DeviceId, Metadata, NodePermission, NodeType};
