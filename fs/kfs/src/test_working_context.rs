...
use unittest::{def_test, TestResult, assert};
...
fn test_working_context_chdir() -> TestResult {
    ...
    let subdir = root_loc.create("testdir", OpenOptions::dir().create(true))
...
fn test_working_context_with_cwd() -> TestResult {
    ...
    // Create a subdirectory
    let subdir = root_loc.create("testdir2", OpenOptions::dir().create(true))
        .expect("Failed to create directory");
