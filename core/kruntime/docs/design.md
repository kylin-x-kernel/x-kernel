...
BootInfo*
    ...
    → final_init
    → init_scheduler
    → init_interrupt (softirq runner、kwork/system_wq、定时器 tick、IPI、PMU 基础 handler)
...
- `init_cb()`：late-init 在 driver probe 和 entropy 初始化后、filesystem namespace 构造前，
  按指针步进调用段内每个 `extern "C" fn()`。
