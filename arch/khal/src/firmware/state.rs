...
static DTB_CAPTURE: LazyInit<(usize, usize)> = LazyInit::new();
...
pub fn dtb_capture_region() -> Option<(usize, usize)> {
...
pub(super) fn init_dtb_capture(paddr: usize, size: usize) {
    if paddr != 0 && size != 0 {
        DTB_CAPTURE.init_once((paddr, size));
    }
}
